...
// MAGIC 
// MAGIC > <b>Example</b>: To create (instantiate) a DataFrame, use this syntax: `val df = ...`.
// MAGIC 
...
// MAGIC 
// MAGIC `df.select(...).groupBy(...).filter(...)`
// MAGIC 
// MAGIC versus
// MAGIC 
// MAGIC `df.filter(...).select(...).groupBy(...)`
...
// MAGIC 
// MAGIC Notice that columns in DataFrames are referenced by `$"<columnName>"`.
// MAGIC 
// MAGIC | SQL                                         | DataFrame (Scala)                          |
// MAGIC | ------------------------------------------- | ------------------------------------------ |
// MAGIC | `SELECT col_1 FROM myTable`                 | `df.select($"col_1")`                      |
// MAGIC | `DESCRIBE myTable`                          | `df.printSchema`                           |
// MAGIC | `SELECT * FROM myTable WHERE col_1 > 0`     | `df.filter($"col_1" > 0)`                  |
// MAGIC | `..GROUP BY col_2`                          | `..groupBy($"col_2")`                      |
// MAGIC | `..ORDER BY col_2`                          | `..orderBy($"col_2")`                      |
// MAGIC | `..WHERE year(col_3) > 1990`                | `..filter(year($"col_3") > 1990)`          |
...
val dordonDF = peopleDF
  .select(year($"birthDate") as "birthYear", $"firstName")
  .filter($"firstName" === "Donna" || $"firstName" === "Dorothy")
  .filter($"gender" === "F")
  .filter(year($"birthDate") > "1990")
  .orderBy("birthYear")
...
val womenBornAfter1990DF = peopleDF
  .select($"firstName", $"middleName", $"lastName",year($"birthDate") as "birthYear", $"salary")
  .filter(year($"birthDate") > 1990)
  .filter($"gender" === "F")
...
// TODO
val top10FemaleFirstNamesDF = // FILL_IN

// COMMAND ----------

val top10FemaleNamesDF = top10FemaleFirstNamesDF.orderBy($"firstName")
...
// MAGIC **A:** The full list is at <a href="https://spark.apache.org/docs/2.1.0/api/java/org/apache/spark/sql/Dataset.html" target="_blank"> org.apache.spark.sql.Dataset</a>.
// MAGIC Look under `Dataset<Row>`, specifically operations such as `filter`, `select`, etc
// MAGIC 
